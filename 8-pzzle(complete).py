...
class FifteenPuzzle:
    def __init__(self, root):
        ...
        # Label to display steps and time
        self.steps_label = tk.Label(self.root, text="Steps: 0", font=('Helvetica', 14), bg="yellow")
        self.steps_label.grid(row=2, column=0, pady=5)

        self.time_label = tk.Label(self.root, text="Time: 0s", font=('Helvetica', 14), bg="yellow")
        self.time_label.grid(row=3, column=0, pady=5)

        self.steps_count = 0  # Biến đếm số bước đi
    ...
    def move(self, direction):
        ...
        self.steps_count += 1  # Tăng số bước đi
        self.update_labels()  # Cập nhật nhãn hiển thị số bước đi
        self.update_buttons()
        if self.check_goal():
    ...
    def update_labels(self):
        self.steps_label.config(text=f"Steps: {self.steps_count}")
    ...
    def show_win_message(self):
        total_time = time.time() - self.start_time if self.start_time else 0  # Tính thời gian hoàn thành
        messagebox.showinfo("8-Puzzle", f"Congratulations! You solved the puzzle!\nTotal Steps: {self.steps_count}\nTime: {total_time:.2f}s")
        self.root.quit()
    ...
    def make_move(self, state, x, y, direction):
        ...
        if direction == 'up' and x < self.size - 1:
            ...
            return None
        return tuple(tuple(row) for row in new_state)  # Trả về trạng thái mới dưới dạng tuple

    def execute_solution(self, path):
        for direction in path:
            self.move(direction)  # Di chuyển theo từng bước trong giải pháp
            self.update_buttons()
            self.update_labels()
            self.root.update()  # Cập nhật giao diện để thấy các bước

        self.show_win_message()  # Hiện thông báo khi hoàn thành
